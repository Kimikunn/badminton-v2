 ...
 * createOrderCheck → createOrder（带签名）→ 落库。
 *
...
async function tryFulfill(intent, date, daySlots, env) {
  ...
  const needTotal = run.reduce((sum, h) => sum + Math.max(0, intent.courts_needed - h.lockedCount), 0);
  if (needTotal > 0) {
    const limit = env.dailyOrderLimit || 2;
    const held = heldOrdersToday();
    if (held + needTotal > limit) {
      await notifyDailyLimit(intent, date, held, needTotal, limit, env);
      return { fulfilled: false, locked: 0, failed: 0, skipped: 'daily_limit' };
    }
  }

  let locked = 0;
  const failedList = []; // { slot, error, riskControl }
  let aborted = false;
  for (const hour of run) {
    ...
    for (const slot of candidates) {
      ...
      if (result.success) {
        ...
        failedList.push({ slot, error: result.error, riskControl: !!result.riskControl });
        // 图形验证是账号级拦截：继续试其他场地没有意义，本轮放弃（fail-fast）
        if (result.riskControl) aborted = true;
      }
    }
  }
...
async function notifyLockFailed(intent, date, failedList, env) {
  ...
  for (const f of failedList) {
    intentService.recordNotification({
      intentId: intent.id,
      uniqNo: f.slot.uniqNo,
      areaName: f.slot.areaName,
      date,
      startTime: f.slot.startTime,
      endTime: f.slot.endTime,
      price: f.slot.price ?? null,
...
async function notifyDailyLimit(intent, date, held, need, limit, env) {
  ...
  const content = `**${date} 锁场暂停：已达当日限订**\n\n场馆限制每账号每天最多订 ${limit} 笔订单，当前已持有 ${held} 笔（含等待支付的），本意图还需 ${need} 笔，额度不足，今天不再自动下单。若有订单超时取消返还额度，系统会自动恢复尝试。`;
